...
export default function DoxaNavbar() {
  ...
  useEffect(() => {
    const loadData = () => {
      // Étape 1 : On simule la récupération de l'utilisateur connecté
      // (Dans ton vrai projet, ça viendra de ton token ou session)
      const currentUser = {
        id: "user_1", // <--- Change ça en "user_2" pour voir les autres notifs !
        name: "Merouane Ait Kaci",
        email: "merouane.dev@doxa.com",
        role: "Administrateur",
        avatarUrl: null 
      };
      ...
      // Étape 2 : On charge les notifications SPÉCIFIQUES à cet ID utilisateur
      // Simulation API : const data = await fetch(`/api/notifications/${currentUser.id}`)
      const userSpecificNotifs = MOCK_NOTIFICATIONS_DB[currentUser.id] || [];
